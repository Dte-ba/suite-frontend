...
export default Ember.Component.extend({
  ...
  responsableComoObjeto: Ember.computed("parametros.responsable", function() {
    if (this.get("parametros.responsable")) {
      return this.get("store").find("perfil", this.get("parametros.responsable"));
    } else {
      return this.get("opcionTodos");
    }
  }),

  participanteComoObjeto: Ember.computed("parametros.participante", function() {
    if (this.get("parametros.participante")) {
      return this.get("store").find("perfil", this.get("parametros.participante"));
    } else {
      return this.get("opcionTodos");
    }
  }),
  ...
  cuandoCambiaDeRegion: Ember.observer("region", function() {
    if (this.get("ultimaRegionSeleccionada") != this.get("region")) {
      Ember.run.later(() => {
        ...
        this.get("accionCompleta")("participante", "");
  ...
  tarea: task(function*() {
    ...
    if (region) {
      ...
      personas.pushObject({ nombre: "Seleccione al menos una región para ver las personas.", disabled: true });
    }
  ...
  actions: {
    ...
    cuandoSeleccionaParticipante(perfil) {
      this.get("accionCompleta")("participante", perfil.get("id"));
    }
  }
});
